...
function setup_vendor() {
    ...
    export ROOT_DIR="$3"
    if [ ! -d "$ROOT_DIR" ]; then
        echo "\$ROOT_DIR must be set and valid before including this script!"
        exit 1
    fi
    ...
    export PRODUCTMK="$ROOT_DIR"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$ROOT_DIR"/"$OUTDIR"/Android.mk
    export BOARDMK="$ROOT_DIR"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ROOT_DIR"/vendor/elixium/build/tools/smali/baksmali.jar
        export SMALIJAR="$ROOT_DIR"/vendor/elixium/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$ELIX_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$ELIX_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$ROOT_DIR"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$ROOT_DIR"/vendor/elixium/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$ROOT_DIR"/"$OUTDIR"/radio
